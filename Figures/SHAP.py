...
labels= {'FEATURE': 'Feature %s',
 'FEATURE_VALUE': 'Bacterial abundance',
...
def summary_plot(shap_values, features=None, feature_names=None, max_display=None, plot_type="dot",
    ...
    if plot_type == "dot":
            ...
            for pos, i in enumerate(feature_order):
                ...
                if vmin == vmax:
                    ...
                    if vmin == vmax:
                        vmin = np.min(values)
                        vmax = np.max(values)
                pl.scatter(shaps, np.ones(shap_values.shape[0]) * pos, s=9, cmap=color, vmin=vmin, vmax=vmax,
                ...
                for i in range(len(xs) - 1):
                    if ds[i] > 0.05 or ds[i + 1] > 0.05:
                        pl.fill_between([xs[i], xs[i + 1]], [pos + ds[i], pos + ds[i + 1]],
                                        [pos - ds[i], pos - ds[i + 1]], color=color,
                                        zorder=2)
        ...
        for i,ind in enumerate(class_inds):
            ...
            pl.barh(
                ...
                color=color, label=class_names[ind]
            )
    ...
    if color_bar and features is not None and plot_type != "bar" and \
        ...
        cb.set_label('Bacterial abundance', size=12, labelpad=0)
    ...
    if plot_type == "bar":
        ...
        pl.xlabel(labels['VALUE']%phenotype, fontsize=13)
    if show:
        pl.show()
